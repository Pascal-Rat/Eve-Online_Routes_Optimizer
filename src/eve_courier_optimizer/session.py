...
from .esi import EsiClient, utc_now
...
from .jsonio import json_bool
from .planning import rank_single_contracts
from .presentation import decorate_plan, execution_payload, scope_payload, snapshot_summary
...
from .service import PlannerService, RoutePlan
from .snapshot import ContractSnapshot, read_snapshot, write_snapshot
from .threat_intel import (
    DEFAULT_THREAT_WINDOW_SECONDS,
    ZkillClient,
)
from .web_options import (
    _nonnegative_int,
    _optional_positive_int,
    allowed_security_bands,
    max_candidates,
    solver_config,
    threat_regions_from_scan_body,
)
from .web_options import (
    constraints as planning_constraints,
)
...
class PlanningSession:
    ...
    def _load_existing_artifacts(self) -> None:
        ...
        if self.plan_path.exists():
            ...
            if (
                ...
                self.plan_payload = decorate_plan(
    ...
    def status(self) -> JsonObject:
        return {
            ...
            "snapshot": snapshot_summary(self.snapshot, self.graph),
            "plan": self.plan_payload,
            "execution": execution_payload(self.execution, self.graph, self.clock()),
    ...
    def scan(self, body: dict[str, object]) -> JsonObject:
        ...
        if region_scope == "all":
            ...
            region_ids = tuple(
                sorted(self.graph.region_ids_for_security_bands(allowed_security_bands(body)))
            )
        elif region_scope == "empire":
            compatible = self.graph.region_ids_for_security_bands(allowed_security_bands(body))
        ...
        snapshot = self.service.scan(
            ...
            threat_region_ids=(
                threat_regions_from_scan_body(body, self.graph) if include_threat else None
            ),
        )
        self._invalidate_plan()
        write_snapshot(self.snapshot_path, snapshot)
        self.snapshot = snapshot
        return {"snapshot": snapshot_summary(self.snapshot, self.graph)}

    def rank(self, body: dict[str, object]) -> JsonObject:
        if self.execution is not None:
            raise ValueError("an execution session already exists; use Replan")
        snapshot = self._require_snapshot()
        constraints = planning_constraints(body, snapshot, self.graph, self.clock())
        prepared = self.service.prepare(
            snapshot,
            constraints,
            max_candidates=max_candidates(body),
        )
        ...
        return {"scope": scope_payload(prepared), "items": items}
    ...
    def solve(self, body: dict[str, object]) -> JsonObject:
        ...
        constraints = planning_constraints(body, snapshot, self.graph, self.clock())
        plan = self.service.solve(
            snapshot,
            constraints,
            max_candidates=max_candidates(body),
            solver_config=solver_config(body),
        )
    ...
    def start_execution(self, body: dict[str, object]) -> JsonObject:
        ...
        self.plan = None
        return {"execution": execution_payload(self.execution, self.graph, self.clock())}
