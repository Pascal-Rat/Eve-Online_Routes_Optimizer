...
@dataclass(frozen=True, slots=True)
class RouteProblem:
    """Eligible couriers, mandatory obligations, and their permitted shortest-path distances."""
    ...
    def from_snapshot(
        ...
        *,
        ...
        contract_scores = _score_feasible_contracts(
            routable_contracts,
            constraints,
        ...
        contract_scores = _apply_candidate_cap(
            contract_scores,
            max_candidates,
            heuristic_reductions,
        )

        eligible_contracts = tuple(score.contract for score in contract_scores)
    ...
    def rank_contracts(self) -> tuple[SingleContractScore, ...]:
        return tuple(
            sorted(
                self.scores,
                key=lambda score: (
    ...
    def restrict_contracts(self, selected_contract_ids: tuple[int, ...]) -> RouteProblem:
        ...
        known_contract_ids = {contract.contract_id for contract in self.contracts}
        ...
        selected_contract_scores = tuple(
            score for score in self.scores if score.contract.contract_id in selected_contract_id_set
        )
        return replace(
            self,
...
def _score_feasible_contracts(
    ...
    for routable_contract in routable_contracts:
        if routable_contract.date_expired <= constraints.snapshot_time:
            safe_reductions["listing_expired"] += 1
            continue
        if routable_contract.reward_units <= 0:
            safe_reductions["nonpositive_reward"] += 1
            continue
        if routable_contract.volume_units > constraints.cargo_capacity_units:
            safe_reductions["volume_exceeds_capacity"] += 1
            continue
        if routable_contract.collateral_units > constraints.collateral_budget_units:
        ...
        if (
            ...
            and initial_locked_collateral_units + routable_contract.collateral_units
        ...
        delivery_window_seconds = routable_contract.days_to_complete * 86_400
        if constraints.collateral_mode is CollateralMode.LOCKED:
            ...
            if earliest_pickup_time >= routable_contract.date_expired:
...
def _apply_candidate_cap(
    ...
    if max_candidates is None or len(scores) <= max_candidates:
        return scores
    ranked = sorted(
        scores,
        key=lambda score: (
    ...
    heuristic_reductions["candidate_cap"] = len(scores) - max_candidates
