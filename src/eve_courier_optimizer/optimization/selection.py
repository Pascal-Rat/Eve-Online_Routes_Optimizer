...
from . import batches, relaxation, route_checks, subsets
...
class SelectionProof:
    selection_cuts: relaxation.SelectionCuts
    master_result: relaxation.SystemRelaxationBound | None = None
    incumbent: VerifiedRoute | None = None
    proven_infeasible: bool = False
    status_name: str | None = None
    iteration_count: int = 0
    learned_infeasibility_cores: tuple[tuple[int, ...], ...] = ()
    learned_reward_cuts: tuple[relaxation.SubsetRewardCut, ...] = ()
...
class ContractSelectionSearch:
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config
        self.selection_cuts = (
            relaxation.build_selection_cuts(prepared)
            if len(prepared.problem.contracts) >= MIN_CONTRACTS_FOR_SELECTION_SEARCH
            else relaxation.SelectionCuts((), ())
        )
        self.incumbent = incumbent
        self.best_upper_bound: int | None = None
        self.latest_master: relaxation.SystemRelaxationBound | None = None
        self.learned_cores: list[tuple[int, ...]] = []
        self.learned_reward_cuts: list[relaxation.SubsetRewardCut] = []
    ...
    def run(self) -> SelectionProof:
        if len(self.prepared.problem.contracts) < MIN_CONTRACTS_FOR_SELECTION_SEARCH:
            return self._proof()
        if self.config.relaxation_time_seconds <= 0:
            self.status = "disabled"
            return self._proof()

        master = relaxation.SystemRelaxationMaster(
            self.prepared, selection_cuts=self.selection_cuts
        )
    ...
    def _search_master_selections(
        self, master: relaxation.SystemRelaxationMaster, deadline: float
    ) -> None:
    ...
    def _search_haul_batches(self, master: relaxation.SystemRelaxationMaster) -> None:
        result = batches.solve_batches(
            self.prepared,
        ...
        if result.upper_bound_units is not None:
            ...
            self.latest_master = relaxation.SystemRelaxationBound(
            ...
            self._learn_reward_ceiling(
                master,
                relaxation.SubsetRewardCut(
                    tuple((c.contract_id, c.reward_units) for c in self.prepared.problem.contracts),
                    self.best_upper_bound,
                ),
            )
        self._hint_master(master)
    ...
    def _check_selection(
        self,
        master: relaxation.SystemRelaxationMaster,
        proposal: relaxation.SystemRelaxationBound,
        deadline: float,
        budget: float,
    ) -> _RoundDecision:
        subset = self.prepared.restrict_contracts(proposal.selected_contract_ids)
        previous_reward = self.incumbent.simulation.total_reward_units if self.incumbent else -1
        result = subsets.solve_subset(subset, max_time_seconds=budget)
        if result is not None:
            ...
            if result.complete:
                if result.upper_bound_units is None:
                    ...
                    return _RoundDecision.STOP
                if result.upper_bound_units < sum(c.reward_units for c in subset.problem.contracts):
                    self._learn_reward_ceiling(
                        master,
                        relaxation.SubsetRewardCut(
                            tuple(
                                (c.contract_id, c.reward_units) for c in subset.problem.contracts
                            ),
        ...
        check = route_checks.check_selection(
            self.prepared,
            self.graph,
    ...
    def _accept_route(
        ...
        candidate = VerifiedRoute.verify(self.prepared.problem, self.graph, visits, selected_ids)
    ...
    def _hint_master(self, master: relaxation.SystemRelaxationMaster) -> None:
    ...
    def _record_master(self, result: relaxation.SystemRelaxationBound) -> None:
    ...
    def _learn_reward_ceiling(
        self, master: relaxation.SystemRelaxationMaster, cut: relaxation.SubsetRewardCut
    ) -> None:
        if cut in self.learned_reward_cuts:
            raise RuntimeError("selection search produced a duplicate reward cut")
        relaxation.add_subset_reward_cut(master.model, master.contract_is_selected, cut)
        self.learned_reward_cuts.append(cut)

    def _learn_conflict(
        self, master: relaxation.SystemRelaxationMaster, contract_ids: tuple[int, ...]
    ) -> None:
