...
class PickupDeliveryModel:
    ...
    def __init__(
        ...
        *,
        ...
        self._hint_selection(selection_hint)
    ...
    def _travel_seconds(
        ...
        jump_count = self.problem.jump_matrix.get((source_system_id, destination_system_id))
    ...
    def _event_travel_seconds(
        ...
        if from_event.system_id is None or to_event.system_id is None:
            return None
        jump_count = self.problem.jump_matrix.get((from_event.system_id, to_event.system_id))
    ...
    def _add_circuit(self) -> None:
        self.contract_is_selected = {
            ...
            for contract in self.problem.contracts
        }
    ...
    def _create_event_state(self) -> None:
        ...
        initial_cargo_load_units = self.problem.initial_cargo_units
        self.cargo = [
        ...
        if self.constraints.max_simultaneous_contracts is not None:
            ...
            initial_active_parcel_count = self.problem.initial_parcel_count
            self.parcels = [
        ...
        initial_locked_collateral_units = self.problem.initial_collateral_units
        if self.constraints.collateral_mode is CollateralMode.ROLLING:
            self.collateral = [
        ...
        if self.constraints.travel.service_seconds > 0:
            ...
            self.model.add(
                self.constraints.travel.service_seconds
                * (
                    self.problem.mandatory_action_count
    ...
    def _propagate_route_resources(self) -> None:
        ...
        self.model.add(
            ...
            + self.constraints.travel.service_seconds
            * (self.problem.mandatory_action_count + 2 * sum(self.contract_is_selected.values()))
        )
    ...
    def _add_contract_constraints(self) -> None:
        ...
        for shipment in self.problem.active_shipments:
    ...
    def _maximize_reward(self) -> None:
        committed_reward_units = self.problem.committed_reward_units
        ...
        add_resource_work_bounds(self.model, self.problem, self.contract_is_selected)
        self.model.maximize(self.total_reward_units)

    def _hint_selection(self, visits: tuple[PlannedVisit, ...]) -> None:
        suggested_contract_ids = frozenset(
            v.contract_id for v in visits if isinstance(v, PlannedAction)
        )
    ...
    def hint(
        ...
        for previous, node in zip(nodes, nodes[1:], strict=False):
            ...
            jumps = (
                ...
                else self.problem.jump_matrix[source.system_id, target.system_id]
            )
            arrival[node] = (
                arrival[previous]
        ...
        reward = sum(s.contract.reward_units for s in self.problem.active_shipments)
        reward += sum(
            item.reward_units for item in self.problem.contracts if item.contract_id in selected_set
        )
