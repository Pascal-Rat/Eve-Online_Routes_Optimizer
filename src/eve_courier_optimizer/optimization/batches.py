...
from .relaxation import integer_upper_bound
from .subsets import SubsetSearchResult


def solve_batches(
    prepared: PreparedProblem, *, max_time_seconds: float, random_seed: int = 0
    ...
    def travel(source: int, target: int | None) -> int | None:
        if target is None:
            return 0
        jumps = prepared.jump_matrix.get((source, target))
