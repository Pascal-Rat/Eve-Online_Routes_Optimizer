...
def test_prepare_problem_applies_declared_and_safe_filters(
    ...
    problem = RouteProblem.from_snapshot(snapshot, tiny_graph, constraints(now))

    assert [item.contract_id for item in problem.contracts] == [1]
    assert dict(problem.scope.policy_exclusions) == {
        "security_policy": 1,
        "unsupported_non_npc_station_endpoint": 1,
    }
    assert dict(problem.scope.safe_reductions) == {
        "listing_expired": 1,
...
def test_candidate_cap_marks_proof_scope_as_truncated(
    ...
    assert not problem.scope.is_untruncated
    assert dict(problem.scope.heuristic_reductions) == {"candidate_cap": 1}
    assert problem.contracts[0].contract_id == 2
...
def test_active_contract_still_visible_in_snapshot_is_not_double_counted(
    ...
    assert not problem.contracts
    assert problem.active_shipments == (active,)
    assert dict(problem.scope.safe_reductions) == {"already_active_commitment": 1}
...
def test_solo_ranking_prefers_reward_rate(now: datetime, tiny_graph: UniverseGraph) -> None:
    ...
    problem = RouteProblem.from_snapshot(snapshot, tiny_graph, constraints(now))
    ranking = problem.rank_contracts()
...
def test_snapshot_sde_mismatch_blocks_proof(now: datetime, tiny_graph: UniverseGraph) -> None:
    ...
    with pytest.raises(ValueError, match="snapshot uses SDE"):
        RouteProblem.from_snapshot(snapshot, tiny_graph, constraints(now))


def test_start_must_exist_and_be_permitted(now: datetime, tiny_graph: UniverseGraph) -> None:
    snapshot = make_snapshot(now)
    with pytest.raises(ValueError, match="not present"):
        RouteProblem.from_snapshot(snapshot, tiny_graph, constraints(now, start_system_id=999))
    with pytest.raises(ValueError, match="excluded"):
        RouteProblem.from_snapshot(snapshot, tiny_graph, constraints(now, start_system_id=4))
...
def test_threat_aware_proof_requires_every_route_reachable_region(
    ...
    with pytest.raises(ValueError, match="does not cover.*Transit Region"):
        RouteProblem.from_snapshot(make_snapshot(now), graph, constraints(now, security=policy))
    ...
    assert problem.scope.eligible_contracts == 0
...
def test_policy_exclusions_distinguish_manual_security_and_activity(
    ...
    problem = RouteProblem.from_snapshot(snapshot, tiny_graph, constraints(now, security=policy))
    assert not problem.contracts
    assert dict(problem.scope.policy_exclusions) == {
...
def test_rolling_mode_filters_listing_that_cannot_be_reached(
    ...
    problem = RouteProblem.from_snapshot(snapshot, tiny_graph, very_slow)
    assert not problem.contracts
    reductions = dict(problem.scope.safe_reductions)
...
def test_rolling_mode_treats_exact_expiry_as_too_late(
    ...
    assert not problem.contracts
    assert dict(problem.scope.safe_reductions)["solo_lower_bound_misses_listing_expiry"] == 1
