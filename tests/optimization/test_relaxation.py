...
from eve_courier_optimizer.application.plan_output import solve_result_to_dict
...
from eve_courier_optimizer.optimization import relaxation as bounds
...
def test_lifted_work_tightens_mixed_load_bound(
    ...
    with monkeypatch.context() as patch:
        ...
        old = bounds.SystemRelaxationMaster(
            prepared, selection_cuts=bounds.SelectionCuts((), ())
        ).solve(max_time_seconds=2)
    new = bounds.SystemRelaxationMaster(
        prepared, selection_cuts=bounds.SelectionCuts((), ())
    ).solve(max_time_seconds=2)
...
def test_master_must_allow_pickup_delivery_system_revisits(
    ...
    assert bounds.build_selection_cuts(prepared).pairs == ()
    assert (
        bounds.SystemRelaxationMaster(prepared).solve(max_time_seconds=2).upper_bound_units == 1000
    )
