...
from tests.application.test_courier_trip import constraints
from tests.conftest import make_contract, make_snapshot
from tests.web.test_server import CourierTransport, planning_payload
...
def test_failed_reset_keeps_live_commitments(
    ...
    assert read_trip(app.trip_path) == state
...
def test_invalid_replan_cannot_refresh_the_saved_snapshot(
    ...
    workspace.start_execution({"confirm_locked_acceptance": True})
    snapshot = workspace.snapshot
    saved_snapshot = workspace.snapshot_path.read_bytes()
    ...
    assert workspace.snapshot_path.read_bytes() == saved_snapshot
